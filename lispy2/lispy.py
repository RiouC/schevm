...
def atom(token):
    ...
    # elif token[0] == '"': return token[1:-1].encode('utf8').decode('unicode_escape')
...
def to_string(x):
    ...
    # elif isa(x, str): return '"%s"' % x.encode('utf8').decode('unicode_escape').replace('"',r'\"')
...
def add_globals(self):
    ...
    self.update({
        ...
        'car':lambda x:x[0], 'cdr':lambda x:x[1:], 'abs': abs, 'append':op.add,
